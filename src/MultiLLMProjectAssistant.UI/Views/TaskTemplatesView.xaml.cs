...
namespace MultiLLMProjectAssistant.UI.Views
{
    public partial class TaskTemplatesView : UserControl
    {
        ...
        {
            // Apply what the user currently sees (even if unsaved).
            var current = BuildFromDetails();
            if (string.IsNullOrWhiteSpace(current.Name)) return;

            // Update the shared request context so Request Builder picks it up.
            try
            {
                ...
                {
                    var existingJson = File.ReadAllText(_requestContextPath);
                    snapshot = JsonSerializer.Deserialize<RequestContextSnapshot>(existingJson) ?? new RequestContextSnapshot();
                }
                else snapshot = new RequestContextSnapshot();

                snapshot.Template = current.Name;
                snapshot.UpdatedAt = DateTimeOffset.UtcNow;
                // Apply template content into the JSON Request editor (overwrites for predictable UX)
                snapshot.JsonRequest = current.Content;
                // Clear previous responses so user sees a fresh workflow
